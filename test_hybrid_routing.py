...
def test_unknown_routes_to_llm(monkeypatch):
    ...
    assistant = object.__new__(assistant_v2.Assistant)
    assistant.process_command = lambda command: [
        {"intent": "UNKNOWN", "target": command}
    ]

    # This mirrors the routing condition used by Assistant V2.
...
def test_known_command_does_not_route_to_llm(monkeypatch):
