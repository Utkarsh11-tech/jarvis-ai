...
class Assistant(BaseAssistant):
    ...
    def _parse_confirmation(command):
        ...
        yes = {"yes", "yeah", "yep", "yup", "sure", "confirm", "confirmed", "do it"}
        no = {"no", "nope", "nah", "cancel", "cancel it", "don't", "do not"}
    ...
    def request_confirmation(self, action, prompt, cancel_response="Cancelled.", action_data=None):
        self._stop_wake_timeout()
    ...
    def handle_command(self, command):
        ...
        # IMPORTANT: process_command() returns an UNKNOWN result rather
        # than an empty list for unsupported/conversational commands.
        # UNKNOWN is therefore the actual LLM fallback signal.
    ...
    def _handle_chrome_profile_response(self, response):
        ...
        if selected_profile is not None:
            profile_directory, profile_data = selected_profile
            self._remember_profile(profile_directory, profile_data.get("name", ""))
        self.conversation.clear()
